...
paths:
  /replicate/v1/predictions:
    post:
      summary: 创建任务 stability-ai/stable-diffusion-inpainting
      deprecated: false
      description: '官方文档: https://replicate.com/stability-ai/stable-diffusion-inpainting'
      tags:
        - Replicate 聚合平台
      parameters:
        - name: Authorization
              ...
              properties:
                input:
                  ...
                  properties:
                    mask:
                      type: string
                      description: 黑白图像，用作遮罩，用于对提供的图像进行修复。白色像素将被修复，黑色像素将被保留。
                    image:
                      type: string
                      description: 用于生成变体的初始图像。将调整为高 x 宽。
                    width:
                      type: integer
                      description: 生成图像的宽度（以像素为单位）。需要为 64 的倍数。默认值：512。
                    height:
                      type: integer
                      description: 生成图像的高度（以像素为单位）。需要为 64 的倍数。默认值：512。
                    prompt:
                      type: string
                      description: 输入提示。
                    scheduler:
